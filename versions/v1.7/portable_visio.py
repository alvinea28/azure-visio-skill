#!/usr/bin/env python3
"""Azure Visio 1.7: noninteractive, offline NEW VSDX/PDF creation without Office.

Python 3.10+ is required, not presumed available in any Copilot product. Run
`python portable_visio.py startup` in the actual execution environment.
Offline PNG rendering uses preinstalled ReportLab and Pillow; no installation.
Bundled assets are discovered beside this script, never downloaded or requested.
Explicit developer SVG mode additionally needs resvg-py; it is not first-run setup.
...
share one layout. Validated pre-rendered PNG artwork is embedded unchanged.
Optional SVG artwork is rendered by resvg at 600 dpi to transparent PNG.
Both preserve gradients and aspect ratio, and are embedded as Foreign
...
OFFICIAL_ARCHIVE_SHA256 = "921594ccd1bf3d9c0a1bd7b6d924e050551a59342f2b353bb74bdcf761c35141"
ICON_TERMS = "https://learn.microsoft.com/azure/architecture/icons/"
CONTRACT = "architecture-pack-v1.6"
VERSION = "1.7.0"
ROUTING_BUDGET_SECONDS = 120
ROUTING_DEADLINE = ContextVar("azure_visio_routing_deadline", default=None)
...
def check_routing_deadline():
    deadline = ROUTING_DEADLINE.get()
    require(deadline is None or time.monotonic() <= deadline,
            "Routing exceeded the 120-second layout budget; simplify crowded geometry or ports "
            "within the three required pages and rerun. No input, download, or fallback is pending.")


def bundled_assets():
    path = safe_path(Path(__file__).absolute().parent / "offline-assets.json")
    require(path.is_file(), "Bundled offline-assets.json is missing from this installed skill. "
            "The host did not supply the complete package; startup is blocked, not waiting for a download.")
    return path
...
def relative_parts(value):
    ...
    require(bool(normalized) and "\x00" not in normalized and not normalized.startswith("/")
...
def load_json(path):
    return json_data(read_bytes(path, MAX_JSON))
...
def offline_archive_paths(manifest_path):
    path = safe_path(manifest_path, must_exist=True)
    manifest = load_json(path)
    object_fields(manifest, ("schemaVersion", "kind", "archives"),
                  ("description", "iconCount", "usableIconCount", "source", "terms"), "offline assets")
    require(type(manifest["schemaVersion"]) is int and manifest["schemaVersion"] == 1
            and manifest["kind"] == "azure-visio-offline-png", "Unsupported offline assets manifest")
    array(manifest["archives"], "offline asset archives", 20)
    require(manifest["archives"], "Offline assets manifest has no archives")
    paths, seen = [], set()
    for archive in manifest["archives"]:
        object_fields(archive, ("file", "sha256"), context="offline asset archive")
        parts = relative_parts(archive["file"])
        require(len(parts) == 1 and parts[0].endswith(".zip"),
                "Offline asset archive must be a companion ZIP filename")
        require(parts[0].casefold() not in seen, "Duplicate offline asset archive")
        seen.add(parts[0].casefold())
        local_path = safe_path(path.parent / parts[0], must_exist=True)
        require(isinstance(archive["sha256"], str)
                and hashlib.sha256(read_bytes(local_path, MAX_ZIP)).hexdigest() == archive["sha256"].lower(),
                f"Offline asset archive SHA256 mismatch: {parts[0]}")
        paths.append(local_path)
    return paths


def read_icon_data(manifest_path):
    path = safe_path(manifest_path, must_exist=True)
    manifest = load_json(path)
    object_fields(manifest, ("schemaVersion", "kind", "catalog", "dataFiles"),
                  ("description", "iconCount", "usableIconCount", "source", "terms"), "offline assets")
    require(type(manifest["schemaVersion"]) is int and manifest["schemaVersion"] == 1
            and manifest["kind"] == "azure-visio-offline-png-json", "Unsupported PNG data manifest")
    array(manifest["dataFiles"], "icon data files", 18)
    require(manifest["dataFiles"], "Offline manifest has no PNG data files")
    files, seen, total = {}, set(), 0
    for record in [manifest["catalog"], *manifest["dataFiles"]]:
        object_fields(record, ("file", "sha256"), context="offline data companion")
        parts = relative_parts(record["file"])
        require(len(parts) == 1 and parts[0].endswith(".json"),
                "Offline data must be a sibling JSON companion, not code or an archive")
        require(parts[0].casefold() not in seen, "Duplicate offline data companion")
        seen.add(parts[0].casefold())
        payload = read_bytes(path.parent / parts[0], 5_000_000)
        total += len(payload)
        require(total <= 10_000_000, "Offline PNG data exceeds the companion budget")
        require(isinstance(record["sha256"], str)
                and hashlib.sha256(payload).hexdigest() == record["sha256"].lower(),
                f"Offline data SHA256 mismatch: {parts[0]}")
        files[parts[0]] = json_data(payload)
    records = files[manifest["catalog"]["file"]]
    array(records, "icon catalog", 10000)
    by_id, used = {}, set()
    for record in records:
        require(isinstance(record, dict), "Invalid catalog entry")
        key = identifier(record.get("id"), "catalog id")
        require(key not in by_id, f"Duplicate catalog id: {key}")
        by_id[key] = record
    artwork, expanded = {}, 0
    for companion in manifest["dataFiles"]:
        data = files[companion["file"]]
        object_fields(data, ("schemaVersion", "kind", "images"), context="PNG data companion")
        require(type(data["schemaVersion"]) is int and data["schemaVersion"] == 1
                and data["kind"] == "azure-visio-png-data" and isinstance(data["images"], dict),
                "Unsupported PNG data companion")
        for key, encoded in data["images"].items():
            require(key in by_id and key not in used, "Duplicate or uncatalogued PNG data ID")
            record = by_id[key]
            require(record.get("usable") is True and record.get("dataFile") == companion["file"],
                    "PNG data does not match its catalog companion")
            require(isinstance(encoded, str) and len(encoded) <= (MAX_PNG + 2) // 3 * 4,
                    "PNG base64 data exceeds byte limit")
            try:
                raw = base64.b64decode(encoded, validate=True)
            except (ValueError, binascii.Error) as exc:
                raise PortableError(f"Invalid PNG base64 data: {key}") from exc
            expanded += len(raw)
            require(len(raw) <= MAX_PNG and expanded <= MAX_EXPANDED, "PNG data exceeds decoded byte limit")
            name = "/".join(relative_parts(record.get("path")))
            require(name.startswith("png/") and name.endswith(".png") and name not in artwork,
                    "Invalid or duplicate PNG data path")
            artwork[name] = raw
            used.add(key)
    require(used == {key for key, record in by_id.items() if record.get("usable") is True},
            "Offline PNG data is missing a usable catalog resource")
    return records, artwork


def read_icon_archives(paths):
    array(paths, "icon archives", 20)
    require(paths, "At least one icon archive is required")
    files, seen = {}, set()
    compressed = expanded = count = 0
    for path in paths:
        data = read_bytes(path, MAX_ZIP)
        compressed += len(data)
        require(compressed <= MAX_ZIP, "Combined icon archives exceed byte limit")
        with zipfile.ZipFile(io.BytesIO(data)) as archive:
            for info in archive.infolist():
                count += 1
                require(count <= 10000, "Icon archive entry limit exceeded")
                require("\x00" not in info.orig_filename, "Unsafe archive filename")
                parts = relative_parts(info.filename)
                name = "/".join(parts)
                require(name.casefold() not in seen, "Duplicate archive destination")
                ...
                require(info.compress_type in (zipfile.ZIP_STORED, zipfile.ZIP_DEFLATED),
                        "Unsupported icon ZIP compression")
                expanded += info.file_size
                require(expanded <= MAX_EXPANDED, "Expanded archive size limit exceeded")
                if info.is_dir():
                    require(name == "png" and info.file_size == 0, "Unexpected icon archive directory")
                    continue
                is_png = len(parts) == 2 and parts[0] == "png" and parts[1].endswith(".png")
                require(name in ("catalog.json", "NOTICE.txt") or is_png,
                        "Offline icon archives may contain only catalog.json, NOTICE.txt and png/*.png data")
                limit = MAX_PNG if is_png else MAX_JSON if name == "catalog.json" else 65536
                require(info.file_size <= limit and
                        info.file_size <= max(1024 * 1024, info.compress_size * 200),
                        "Icon archive entry size/expansion limit exceeded")
                payload = archive.read(info)
                require(len(payload) == info.file_size, "Icon archive entry size mismatch")
                files[name] = payload
    require("catalog.json" in files, "Icon archive catalog.json is missing")
    return json_data(files["catalog.json"]), files


def safe_png(data, entry):
    from PIL import Image

    require(len(data) <= MAX_PNG and data.startswith(b"\x89PNG\r\n\x1a\n"), "Invalid bounded PNG icon")
    require(isinstance(entry.get("originalSha256"), str) and
            re.fullmatch(r"[0-9a-fA-F]{64}", entry["originalSha256"]) is not None,
            "PNG icon requires original SVG SHA256 provenance")
    original = relative_parts(entry.get("originalPath"))
    require(original[-1].lower().endswith(".svg"), "PNG icon requires original SVG path provenance")
    aspect = number(entry.get("sourceAspect"), "PNG source aspect", 0.02, 50)
    try:
        with Image.open(io.BytesIO(data)) as image:
            require(image.format == "PNG" and not getattr(image, "is_animated", False),
                    "Only static PNG artwork is supported")
            require(0 < image.width <= 2400 and 0 < image.height <= 2400, "PNG icon dimensions exceed limit")
            for key, actual in (("pixelWidth", image.width), ("pixelHeight", image.height)):
                require(type(entry.get(key)) is int and entry[key] == actual,
                        f"PNG {key} does not match catalog")
            require(abs(image.width / image.height / aspect - 1) < 0.01,
                    "PNG aspect differs from original SVG viewport")
            image.verify()
        with Image.open(io.BytesIO(data)) as image:
            image.load()
            require(image.convert("RGBA").getchannel("A").getbbox() is not None, "PNG icon is empty")
    except (OSError, SyntaxError, Image.DecompressionBombError) as exc:
        raise PortableError(f"Invalid PNG artwork: {exc}") from exc
    return aspect


class IconCatalog:
    def __init__(self, directory=None, *, archives=None, offline_assets=None):
        require(sum(value is not None for value in (directory, archives, offline_assets)) == 1,
                "Use either an icon directory, icon archives, or an offline assets manifest")
        self.directory = safe_path(directory) if directory is not None else None
        self.archive_files = None
        manifest = load_json(offline_assets) if offline_assets is not None else None
        require(manifest is None or isinstance(manifest, dict), "Offline assets manifest must be an object")
        if manifest is not None and manifest.get("kind") == "azure-visio-offline-png-json":
            records, self.archive_files = read_icon_data(offline_assets)
        elif offline_assets is not None:
            records, self.archive_files = read_icon_archives(offline_archive_paths(offline_assets))
        elif archives is not None:
            records, self.archive_files = read_icon_archives(archives)
        else:
        ...
        if self.archive_files is not None:
            ...
            for entry in records:
                if entry.get("usable") is True:
                    name = "/".join(relative_parts(entry.get("path")))
                    require(name.startswith("png/") and name.endswith(".png"),
                            "Offline catalog requires PNG resource paths")
                    require(name in self.archive_files, f"Missing offline PNG resource: {name}")
                    referenced.add(name)
            require(set(self.archive_files) - {"catalog.json", "NOTICE.txt"} <= referenced,
                    "Uncatalogued offline PNG resource")
    ...
    def resolve(self, icon_ref):
        ...
        require(suffix in (".svg", ".png"), "Only vetted SVG or PNG catalog resources are supported")
        if self.archive_files is not None:
            name = "/".join(parts)
            require(name in self.archive_files, f"Missing offline icon resource: {name}")
            data = self.archive_files[name]
        else:
            ...
            data = read_bytes(path, MAX_PNG if suffix == ".png" else MAX_SVG)
        ...
        if suffix == ".png":
            aspect = safe_png(data, entry)
            artwork = {"png": data, "pixels": max(entry["pixelWidth"], entry["pixelHeight"])}
        else:
            source, aspect = safe_svg(data)
            artwork = {"svg": source}
        self.cache[icon_ref] = {**artwork, "aspect": aspect, "sha256": entry["sha256"],
                               "path": entry["path"], "format": suffix[1:]}
        return self.cache[icon_ref]
    ...
    def search(self, query=""):
        ...
        for key, entry in sorted(self.entries.items()):
            haystack = re.findall(r"[a-z0-9]+", (key + " " + str(entry.get("name", ""))).casefold())
            if all(any(word.startswith(token) for word in haystack) for token in tokens):
                results.append({"id": key, "name": entry.get("name", key),
                                "usable": entry.get("usable") is True, "issue": entry.get("issue", ""),
                                "format": PurePosixPath(str(entry.get("path", ""))).suffix.lstrip(".")})
        return results
...
def capabilities():
    ...
    return {"python": sys.version.split()[0], "platform": sys.platform,
            "version": VERSION, "dependencies": available,
            "renderAvailable": available["reportlab"] and available["Pillow"],
            "renderModes": {"png": available["reportlab"] and available["Pillow"],
                            "svg": all(available.values())},
            ...
            "note": "PNG rendering uses preinstalled ReportLab/Pillow and a vetted local PNG catalog. "
                    "SVG additionally needs resvg-py. No installation or host permission is granted."}


def dependencies(*, svg=False):
    available = capabilities()["dependencies"]
    required = ["reportlab", "Pillow"] + (["resvg-py"] if svg else [])
    ...
    require(not missing, "Unavailable rendering dependencies: " + ", ".join(missing)
            + ". Use preinstalled libraries and a vetted PNG catalog where installation is prohibited; "
              "no installation or handoff is performed.")
    try:
        ...
        raise PortableError(f"Preinstalled rendering library could not load: {exc}. "
                            "No installation, download, or user-input wait is attempted.") from exc
    return pdfmetrics, Canvas, ImageReader, resvg_py


def startup(offline_assets=None):
    dependencies()
    require(importlib.util.find_spec("portable_reference") is not None,
            "Bundled portable_reference.py is missing; the installed skill is incomplete")
    manifest_path = bundled_assets() if offline_assets is None else safe_path(offline_assets, must_exist=True)
    catalog = IconCatalog(offline_assets=manifest_path)
    usable = [key for key, entry in catalog.entries.items() if entry.get("usable") is True]
    require(usable, "The bundled catalog contains no usable PNG icons")
    for key in usable:
        require(catalog.resolve(key)["format"] == "png",
                "Default startup requires bundled PNG artwork, not an SVG dependency")
    return {"status": "ready", "version": VERSION, "offlineAssets": str(manifest_path),
            "icons": len(usable), "renderMode": "png", "networkRequired": False,
            "installationRequired": False, "userInputRequired": False,
            "note": "These libraries and bundled resources are ready. Host execution/output permissions "
                    "still apply; this does not authorize them."}
    ...
    for page in model["pages"]:
        ...
        for layout in nodes.values():
            if layout["glyph"]:
                ...
                if art["format"] == "png":
                    images[ref] = {"data": art["png"], "pixels": art["pixels"], "sourceFormat": "png"}
                    continue
                ...
                if ref not in images or images[ref]["pixels"] < pixels:
                    _, _, _, resvg = dependencies(svg=True)
                    ...
                    images[ref] = {"data": png, "pixels": pixels, "sourceFormat": "svg"}
...
def render(model, icon_directory, output_directory, name="architecture", *, model_path=None,
           contract_path=None, bundle=False, icon_archives=None, offline_assets=None):
    ...
    catalog = IconCatalog(icon_directory, archives=icon_archives, offline_assets=offline_assets) if (
        icon_directory is not None or icon_archives is not None or offline_assets is not None) else None
    validate_resources(model, catalog)
    token = ROUTING_DEADLINE.set(time.monotonic() + ROUTING_BUDGET_SECONDS)
    try:
        layouts, images = prepare(model, catalog)
        check_routing_deadline()
    finally:
        ROUTING_DEADLINE.reset(token)
    ...
    try:
        ...
        destination.rmdir()
        raise
    return {"status": "complete", "version": VERSION, "pageCount": len(inspection["pages"]),
            "vsdx": str(destination / (name + ".vsdx")),
            ...
            "glyphFormat": ("Packaged PNG pixels unchanged within editable native groups"
                            if any(image["sourceFormat"] == "png" for image in images.values())
                            else "600dpi transparent PNG within editable native groups"),
...
def download_icons(output_directory, accept_icon_terms=False):
    ...
    request = urllib.request.Request(OFFICIAL_URL, headers={"User-Agent": "AzureVisio-Portable/1.6.1"})
...
def automatic_destination(model_path, name, output_root=None):
    identifier(name, "output basename")
    root = safe_path(output_root) if output_root is not None else safe_path(model_path, must_exist=True).parent
    resources = Path(__file__).absolute().parent
    require(root.is_dir(), "Output root must be an existing writable task folder")
    require(root != resources and not root.is_relative_to(resources),
            "Automatic output cannot be written into installed skill resources. The host must supply "
            "a writable task folder for --output-root; do not ask for icons or downloads.")
    return root / (name + "-" + uuid.uuid4().hex)
...
def main(argv=None):
    ...
    p = commands.add_parser("startup", help="Check actual imports and all bundled icons without input or downloads")
    p.add_argument("--offline-assets", help="Optional explicit manifest; defaults beside this script")
    for command in ("validate", "render", "catalog"):
        p = commands.add_parser(command)
        if command != "catalog":
            p.add_argument("--model", required=True)
        else:
            p.add_argument("--search", default="", help="Filter actual icon names/IDs; never guess an ID")
        source = p.add_mutually_exclusive_group()
        source.add_argument("--icon-directory")
        source.add_argument("--icon-archive", action="append", help="Data-only PNG icon ZIP; repeat for shards")
        source.add_argument("--offline-assets", help="Bundled manifest with exact PNG archive filenames/hashes")
        if command == "render":
            output = p.add_mutually_exclusive_group()
            output.add_argument("--output-directory", help="Explicit NEW output directory")
            output.add_argument("--output-root", help="Existing task folder for automatically named output")
    ...
    p = commands.add_parser("import-icons", help="Build an SVG catalog from a local official Azure ZIP; no network")
    ...
    try:
        if args.command == "startup":
            result = startup(args.offline_assets)
        ...
        elif args.command == "import-icons":
            require(args.accept_icon_terms, f"Read {ICON_TERMS} and accept its permitted-use terms")
            data = read_bytes(args.archive, MAX_ZIP)
            require(hashlib.sha256(data).hexdigest() == OFFICIAL_ARCHIVE_SHA256,
                    "Local archive is not the verified Azure V24 source; refusing unverified official provenance")
            result = extract_official_archive(data, args.output_directory)
        else:
            if args.icon_directory is None and args.icon_archive is None and args.offline_assets is None:
                args.offline_assets = bundled_assets()
            if args.command == "catalog":
                result = IconCatalog(args.icon_directory, archives=args.icon_archive,
                                     offline_assets=args.offline_assets).search(args.search)
            else:
                model = load_json(args.model)
                if args.command == "validate":
                    result = validate_model(model, args.model)
                    result["resolvedIcons"] = validate_resources(
                        model, IconCatalog(args.icon_directory, archives=args.icon_archive,
                                           offline_assets=args.offline_assets))
                else:
                    destination = args.output_directory or automatic_destination(
                        args.model, args.name, args.output_root)
                    result = render(model, args.icon_directory, destination, args.name,
                                    model_path=args.model, bundle=args.bundle, icon_archives=args.icon_archive,
                                    offline_assets=args.offline_assets)
        print(json.dumps(result, ensure_ascii=False, indent=2))
        return 0
    except (PortableError, OSError, zipfile.BadZipFile, ET.ParseError, urllib.error.URLError) as exc:
        print(json.dumps({"status": "blocked", "valid": False, "error": str(exc),
                          "comUsed": False, "userInputPending": False}), file=sys.stderr)
        return 2
